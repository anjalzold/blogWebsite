...
const Button = ({
  ...
  return (
    <button
      className={`${className} flex items-center justify-center text-center cursor-pointer  || ""} ${(size && sizes[size]) || ""} ${(variant && variants[variant]?.[color]) || ""}`}
      {...restProps}
    >
      {!!leftIcon && leftIcon}
      {children}
